...
!function ($) {
	 ...
	 **/
    bTabs.prototype.addTabWithParams = function (id, title, url){
    ...
    */
	...
	bTabs.prototype.closeTab = function(id){
        ...
        if (prevLi.innerText != "")$('a', $(prevLi)).tab('show');
	};
    ...
    /**
	 * bTabsAddbWithParams新增标签页
	 */
    function bTabsAddbWithParams(id, title, url) {
        return this.each(function () {
            if (!id || !title || !url) return;
            var $this = $(this), data = $this.data('bTabs');
            if (data) data.addTabWithParams(id, title, url);
        });
    }
	/**
	 * 关闭标签页
    ...
    $.fn.bTabsAddbWithParams = bTabsAddbWithParams;
